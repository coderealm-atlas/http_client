...
// Returns the PEM certificate used by the local HTTPS test server
inline const std::string& certificate_pem() {
...
inline const std::string& dh_pem() {
...
// Load the server certificate, private key and DH params into an ssl::context
inline void load_server_certificate(boost::asio::ssl::context& ctx) {
  ...
  */
  ...
  const auto& cert = certificate_pem();
  const auto& key = private_key_pem();
  const auto& dh = dh_pem();
